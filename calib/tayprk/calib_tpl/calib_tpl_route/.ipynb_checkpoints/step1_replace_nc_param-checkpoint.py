...
route_param_tmp = 'REPLACE_CALIB_DIR/param.nml.default.tpl'
route_param = 'REPLACE_CALIB_DIR/model/route/ancillary_data/param.nml.default'
#===Modelers need to change this part according to their case study===
...
# replace route relevant parameters
if path.exists(route_param):
    remove(route_param)
with open(route_param_tmp, 'r+') as main:
    with open(route_param, 'w') as new_main:
        input_data = main.read()
        for i in [-2, -1]:
            param_name = param_names[i]
            param_value = param_values[i]
            input_data = input_data.replace(param_name, str(param_value))
